...
def process_image(input_path, output_path, pass_count=1, intensify_passes=1, threshold=0.5, multiplier=2, power=2):
    raw_rgb = misc.imread(input_path)
    membership = fuzzify(raw_rgb)
    for pass_num in xrange(1, pass_count + 1):
        membership = membership_pass(membership, intensify_passes, threshold, multiplier, power)
